...
ActiveSupport.on_load(:fat_free_crm_contact) do
  ...
  class_eval do
    ...
    # If subgroup_only is checked, ensures list cannot be saved without at least one subgroup selected.
    # ---------------------------------------------------------
    validate do
      if FfcrmMailchimp.config.subgroups_only?
        FfcrmMailchimp.config.mailchimp_list_fields.each do |field|
          ...
          if subscription.has_list? and !subscription.has_at_least_one_group?
